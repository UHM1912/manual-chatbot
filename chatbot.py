...
# -----------------------
# DISPLAY CHAT WITH METRICS
# -----------------------
for i, msg in enumerate(st.session_state.chat_history):
    with st.chat_message(msg["role"], avatar="👤" if msg["role"] == "user" else "🤖"):
        st.markdown(msg["content"])
        
        # Show metrics only for assistant messages with metadata
        if msg["role"] == "assistant" and "confidence" in msg:
            ...
            with col1:
                ...
                icon = confidence_map.get(msg["confidence"], "⚪")
                st.markdown(f"""
                <div class="metric-card">
                    <div class="metric-label">📊 Confidence Level</div>
                    <div class="metric-value">{icon} {msg["confidence"]}</div>
                </div>
                """, unsafe_allow_html=True)
            ...
            with col2:
                ...
                <div class="metric-card">
                    ...
                    <div class="metric-value">{msg["similarity_score"]:.3f}</div>
                </div>
                """, unsafe_allow_html=True)
